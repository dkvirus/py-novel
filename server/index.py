...
根据关键字查询小说名/作者名
'''
@app.route('/api/novel/search/<name>')
def search (name):
...
根据小说url地址查询章节目录
'''
@app.route('/api/novel/chapter/<path:url>')
def chapter (url):
    try:
...
点击某一章节，返回章节内容
'''
@app.route('/api/novel/content/<path:url>')
def content (url):
    try:
        content_source = requests.get(url).text
        reg = r'<h1>(.*?)</h1>.*?<div id="content">(.*?)</div>.*?</a>.*?<a href="(.*?)">上一章</a>.*?章节目录</a>.*?<a href="(.*?)">下一章</a>'
